...
async function initialize() {
  try {
    ...
    if (typeof showToast === "function")
      showToast("Initialization failed — check console", "error", 5000);
  }
  copyResBtn.textContent = "Copy Response Meta";
  ...
  personaCheckbox.addEventListener("change", () => {
    try {
      localStorage.setItem(
        "chet_persona_enabled",
        personaCheckbox.checked ? "true" : "false",
      );
    } catch (e) {}
    showToast(
      personaCheckbox.checked
        ? "Playful persona enabled"
        : "Playful persona disabled",
      "info",
      1200,
    );
  });
  ...
  openInspectorBtn.addEventListener("click", () => {
    ...
    openInspectorBtn.textContent = visible
      ? "Open Request Inspector"
      : "Hide Request Inspector";
    try {
      localStorage.setItem("chet_inspector_visible", (!visible).toString());
    } catch (e) {}
  });
  ...
  window.__chetInspector = {
    setRequest: (payload) => {
      try {
        inspectorRequest.textContent = JSON.stringify(payload, null, 2);
      } catch (e) {
        inspectorRequest.textContent = String(payload);
      }
    },
    setResponseMeta: (meta) => {
      try {
        inspectorResponse.textContent = JSON.stringify(meta, null, 2);
      } catch (e) {
        inspectorResponse.textContent = String(meta);
      }
    },
  };
  ...
  copyReqBtn.addEventListener("click", () => {
    try {
      navigator.clipboard
        .writeText(inspectorRequest.textContent || "")
        .then(() => showToast("Request copied to clipboard", "success", 1400));
    } catch (e) {
  ...
  copyResBtn.addEventListener("click", () => {
    try {
      navigator.clipboard
        .writeText(inspectorResponse.textContent || "")
        .then(() => showToast("Response meta copied", "success", 1400));
    } catch (e) {
      showToast("Copy failed", "error", 1400);
    }
  });
...
function setupTheme() {
  ...
  try {
    ...
    const bodyTheme = Array.from(document.body.classList).find(
  ...
  themeSelect.addEventListener("change", (e) => {
    ...
    showToast(
      `Theme set to ${themeSelect.options[themeSelect.selectedIndex].text}`,
      "success",
      1000,
    );
  });
...
function setupServerMetaModal() {
  ...
  modelAlert.addEventListener("click", () => {
    ...
    try {
      metaPre.textContent = JSON.stringify(lastServerMeta, null, 2);
    } catch (e) {
      metaPre.textContent = String(lastServerMeta);
    }
  ...
  copyBtn.addEventListener("click", () => {
    try {
      navigator.clipboard.writeText(metaPre.textContent || "");
      showToast("Server meta copied", "success", 1200);
    } catch (e) {
...
function setupAccordionSections() {
  ...
  sectionToggles.forEach((toggle) => {
    ...
    const toggleHandler = (e) => {
      ...
      if (isCollapsed) {
        ...
        showToast(
          `Collapsed ${toggle.textContent.trim()} section`,
          "info",
          900,
        );
  ...
  window.addEventListener("resize", () => {
    document
      .querySelectorAll(".sidebar-section:not(.collapsed)")
      .forEach((section) => {
        section.style.maxHeight = section.scrollHeight + "px";
      });
  });
...
function setupEventListeners() {
  ...
  temperatureSlider.addEventListener("input", () => {
    updateParameterDisplays();
    showToast(
      `Temperature: ${parseFloat(temperatureSlider.value).toFixed(1)}`,
      "info",
      1500,
    );
  });
  topPSlider.addEventListener("input", () => {
    updateParameterDisplays();
    showToast(
      `Top P: ${parseFloat(topPSlider.value).toFixed(2)}`,
      "info",
      1500,
    );
  });
  ...
  repetitionPenaltySlider.addEventListener("input", () => {
    updateParameterDisplays();
    showToast(
      `Repetition penalty: ${parseFloat(repetitionPenaltySlider.value).toFixed(1)}`,
      "info",
      1500,
    );
  });
  frequencyPenaltySlider.addEventListener("input", () => {
    updateParameterDisplays();
    showToast(
      `Frequency penalty: ${parseFloat(frequencyPenaltySlider.value).toFixed(1)}`,
      "info",
      1500,
    );
  });
  presencePenaltySlider.addEventListener("input", () => {
    updateParameterDisplays();
    showToast(
      `Presence penalty: ${parseFloat(presencePenaltySlider.value).toFixed(1)}`,
      "info",
      1500,
    );
  });
...
function selectModel(modelKey) {
  ...
  modelInfo.style.background =
    "linear-gradient(135deg, #B3E5FC 0%, #E1F5FE 100%)";
  ...
  small.appendChild(
    document.createTextNode(
      " " + (model.maxTokensMax?.toLocaleString() ?? "-") + " tokens",
    ),
  );
  ...
  setTimeout(() => {
    modelInfo.style.background =
      "linear-gradient(135deg, var(--assistant-msg-bg) 0%, #E1F5FE 100%)";
  }, 2000);

  // Update current model indicator
  currentModelDisplay.textContent = `Model: ${model.name}`;
  currentModelDisplay.style.background =
    "linear-gradient(135deg, #00BCD4 0%, var(--accent-color) 100%)";
  setTimeout(() => {
    currentModelDisplay.style.background =
...
function updateParameterRanges(model) {
  ...
  frequencyPenaltySlider.min =
    modelKey.includes("deepseek") || modelKey.includes("hermes") ? -2 : 0;
  frequencyPenaltySlider.max = 2;
  frequencyPenaltySlider.value = 0;

  // Presence penalty (ranges vary by model)
  presencePenaltySlider.min =
    modelKey.includes("deepseek") || modelKey.includes("hermes") ? -2 : 0;
...
function updateParameterDisplays() {
  ...
  const repetitionPenalty = parseFloat(repetitionPenaltySlider.value).toFixed(
    1,
  );
...
function getCurrentParameters() {
  const params = {
    model:
      currentModel && currentModel.key ? currentModel.key : modelSelect.value,
...
async function sendMessage() {
  ...
  try {
    ...
    console.log(
      "Sending chat with parameters:",
      parameters,
      "modelConfigId:",
      availableModels[parameters.model]?.id,
    );

    // Send inspector payload for dev tools (if enabled)
    if (
      window.__chetInspector &&
      typeof window.__chetInspector.setRequest === "function"
    ) {
      try {
        window.__chetInspector.setRequest({
    ...
    if (!parameters.model || !availableModels[parameters.model]) {
      ...
      if (keys.length > 0) {
        ...
        console.warn(
          `Model "${parameters.model}" not available, falling back to ${fallback}`,
        );
        parameters.model = fallback;
        try {
          showToast(
            `No valid model selected — falling back to ${availableModels[fallback].name}`,
            "warning",
            3500,
          );
        } catch (e) {}
        ...
        showToast(
          "No models are available. Cannot send message.",
          "error",
          4000,
        );
        throw new Error("No models available");
    ...
    async function fetchChatWithFallback(b64url) {
      ...
      if (!resp.ok) {
        ...
        const likelyProxy =
          resp.status === 400 && /invalid json|payloadb64/i.test(errText || "");
    ...
    try {
      chatMessages.style.setProperty("overflow-anchor", "none");
    } catch (e) {}
    ...
    if (!response.ok) {
      ...
      const message = bodyText
        ? `Server error: ${bodyText}`
        : `HTTP ${response.status}: Failed to get response`;
      ...
      while ((newlineIndex = sseBuffer.indexOf("\n")) !== -1) {
        ...
        try {
          ...
          if (jsonData.response) {
            ...
            if (
              window.__chetInspector &&
              typeof window.__chetInspector.setResponseMeta === "function"
            ) {
              try {
                window.__chetInspector.setResponseMeta(serverMeta);
              } catch (e) {}
            }
          }
        } catch (e) {
    ...
    if (serverMeta) {
      ...
      if (serverMeta.modelKey !== parameters.model) {
        ...
        validation.status = "warning";
        validation.reason =
          "Response was very short or empty. Might indicate a problem.";
      } else {
      ...
      if (
        ...
        try {
          window.__chetInspector.setResponseMeta({
    ...
    try {
      if (metaEl && metaEl.appendChild) metaEl.appendChild(statusEl);
    } catch (e) {}
    ...
    try {
      ...
      if (
        serverMeta &&
        serverMeta.modelKey &&
        serverMeta.modelKey !== parameters.model
      ) {
    ...
    try {
      personaEnabled =
        !!document.getElementById("persona-enabled")?.checked ||
        localStorage.getItem("chet_persona_enabled") === "true";
    } catch (e) {}
    if (personaEnabled) {
      const greetingText =
        "Hey there — C.H.E.T. here, feeling a bit whimsical today. Here's what I think:\n\n";
    ...
    try {
      chatMessages.style.removeProperty("overflow-anchor");
    } catch (e) {}
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (savedPrompts.length > 0) {
        showToast(
          `Loaded ${savedPrompts.length} saved prompts`,
          "success",
          2000,
        );
      }
