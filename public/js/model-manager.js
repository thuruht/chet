...
class ModelManager {
  ...
  async loadModels() {
    if (window.showToast)
      window.showToast("Loading available models...", "info", 1000);
    try {
      ...
      if (this.modelInfo) {
        while (this.modelInfo.firstChild)
          this.modelInfo.removeChild(this.modelInfo.firstChild);
      ...
      if (window.showToast)
        window.showToast("Failed to load models", "error", 3000);
    }
  }
  ...
  renderModelInfo(model) {
    ...
    tempSpan.textContent =
      "Temp: " +
      (model.temperatureMin !== undefined
        ? model.temperatureMin + "-" + model.temperatureMax
        : "N/A");
    stats.appendChild(tempSpan);

    const topPSpan = document.createElement("span");
    topPSpan.textContent =
      "Top-P: " +
      (model.topPMin !== undefined
        ? model.topPMin + "-" + model.topPMax
        : "N/A");
    stats.appendChild(topPSpan);

    const topKSpan = document.createElement("span");
    topKSpan.textContent =
      "Top-K: " +
      (model.topKMin !== undefined
        ? model.topKMin + "-" + model.topKMax
        : "N/A");
  ...
  updateParameterRanges(model) {
    ...
    if (!controls) return;
    if (controls.maxTokens && controls.maxTokens.slider) {
      controls.maxTokens.slider.min = 256;
      controls.maxTokens.slider.max = model.maxTokensMax || 4096;
      controls.maxTokens.slider.value =
        model.maxTokensDefault || controls.maxTokens.slider.min;
    }
    if (controls.temperature && controls.temperature.slider) {
      controls.temperature.slider.min = model.temperatureMin || 0;
      controls.temperature.slider.max = model.temperatureMax || 2;
      controls.temperature.slider.value =
        model.temperatureDefault || controls.temperature.slider.min;
    }
    if (controls.topP && controls.topP.slider) {
      controls.topP.slider.min = model.topPMin || 0;
      controls.topP.slider.max = model.topPMax || 1;
      controls.topP.slider.value =
        model.topPDefault || controls.topP.slider.min;
    }
    if (controls.topK && controls.topK.slider) {
      controls.topK.slider.min = model.topKMin || 0;
      controls.topK.slider.max = model.topKMax || 100;
      controls.topK.slider.value =
        model.topKDefault || controls.topK.slider.min;
    }
  }
  ...
  updateParameterDisplays() {
    Object.entries(this.parameterControls).forEach(([param, controls]) => {
      ...
      else if (param === "topP") value = Number(value).toFixed(2);
  ...
  showParameterFeedback(param, value) {
    ...
    const messages = {
      ...
      temperature: "Temperature: " + Number(value).toFixed(1),
      topP: "Top P: " + Number(value).toFixed(2),
      topK: "Top K: " + value,
    };
  ...
  getCurrentParameters() {
    ...
    Object.entries(this.parameterControls).forEach(([param, controls]) => {
      ...
      if (param === "maxTokens" || param === "topK")
        params[param] = parseInt(raw);
      else params[param] = raw;
    });
    if (this.seedInput && this.seedInput.value && this.seedInput.value.trim())
